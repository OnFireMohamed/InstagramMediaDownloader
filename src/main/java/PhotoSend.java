...
public class PhotoSend {
    ...
    void System(String Url, String username, String senderusername, String threadid){
        try {
            ...
            Upload(bytes, time);
            Send(threadid, time);
            System.out.println("Avatar of : @" + username + " has been saved by : @" + senderusername + " At : " + new Date(System.currentTimeMillis()));
        }
        catch (Exception e){
    ...
    private void Upload(byte[] photoBytes, String time){
    ...
    private void Send(String threadid, String time){
        Requests client = new Requests();
        client.AddHeader("User-Agent", this.UserAgent);
        client.AddHeader("Host", "i.instagram.com");
